...
import { insertImport, addDeclarationToModule } from '@schematics/angular/utility/ast-utils';
...
function addDeclarationAndExportToModule(modulePath: string, symbolName: string, importPath: string): Rule {
  return (host: Tree) => {
    const text = host.read(modulePath);
    if (!text) {
      throw new Error(`File ${modulePath} does not exist.`);
    ...
    const declarationChanges = addDeclarationToModule(sourceFile, modulePath, symbolName, importPath);
...
export default function (options: any): Rule {
  return (_tree: Tree, _context: SchematicContext) => {
    ...
    const className = strings.classify(`${options.name}TableGridComponent`);
    const dasherizedName = strings.dasherize(options.name).concat('-table-grid');
    const componentName = strings.dasherize(options.name);
    const importPath = `${options.path}/${componentName}/${dasherizedName}/${dasherizedName}.component`.replace('src/app/', './'); // Corrigir o caminho de importação

    return chain([
      mergeWith(templateSource),
      addImportToModule(modulePath, className, importPath),
      addDeclarationAndExportToModule(modulePath, className, importPath),
    ]);
  };
}
